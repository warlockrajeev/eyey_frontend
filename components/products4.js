...
export default function Products4() {
  ...
  useEffect(() => {
    const fetchPocketProducts = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
        const res = await fetch(`${apiUrl}/api/products?premiumInPocket=true&limit=6`);
        if (!res.ok) throw new Error("Failed to fetch premium in pocket products");
        ...
        if (data.products && data.products.length > 0) {
          const formatted = data.products.map((p) => {
            ...
            const catSlug = (p.category || "computer-glasses").toLowerCase().replace(/\s+/g, "-");
            return {
              _id: p._id,
              name: p.name,
              price: p.price,
        ...
        console.error("Error loading Premium in your Pocket:", err);
        setProducts(staticFallback);
      } finally {
        setLoading(false);
      }
