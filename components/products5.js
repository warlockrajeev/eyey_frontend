...
export default function Products5() {
  ...
  useEffect(() => {
    const fetchSeasonsLatest = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
        const res = await fetch(`${apiUrl}/api/products?seasonsLatest=true&limit=6`);
        if (!res.ok) throw new Error("Failed to fetch Season's Latest Collection");
        ...
        console.error("Error loading Season's Latest Collection:", err);
        setProducts(staticFallback);
      } finally {
        setLoading(false);
      }
